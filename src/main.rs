        ...
        Some((command_name, sub_matches)) => {
            match command_name {
                ...
                "post" => {
                    let config_path = sub_matches.get_one::<String>("config").unwrap();
                    let debug = sub_matches.get_flag("debug");
                    let mode = get_fetch_mode(sub_matches.get_flag("resume"), sub_matches.get_flag("refresh"));
...
async fn handle_post_mode(config_path: &str, debug: bool, mode: FetchMode) {
    let config = load_config(config_path);
    let browser = open_page::open_browser(config.headless, debug, &config.browser_data_path.as_str())
        .unwrap();
    let tab = browser.new_tab().unwrap();
    tab.navigate_to("https://www.bilibili.com").unwrap();
    ...
    fetch_posts::fetch_post_details_from_browser(
        &tab,
        &result_db,
        &runtime_db,
        &pending_posts,
...
async fn handle_reply_mode(config_path: &str, debug: bool, mode: FetchMode) {
    ...
    fetch_reply::fetch_replies_from_browser(
        &tab,
        &result_db,
        &runtime_db,
        &config,
